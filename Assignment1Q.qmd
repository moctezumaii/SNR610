...
You should not answer these questions until you have completed the @sec-ass1 section
:::
...
1.  Create a new Quarto file (html). This is the file that you will submit.
2.  Write an explanation of what you think a statistical model is
3.  Inside the Quarto file, using a code chunk, load the dataset, and then remove the first column `X` as we did earlier. This was explained in @sec-ass1
4.  Look at the whole dataset
5.  In @sec-ass1 you were asked to think about a plot. If you did your plot in R, try to embed it using code. If you used a different software option (or drew it), upload it as an independent file.
:::
...
Tip: In the three objects you created, one for each species, add a column that estimates this BCI. Report the maximum and the minimum BCI, using the `max` and `min` functions, for each species.
:::

Click Render, and submit the resulting html file. If you can't get it to work, ask for help.
