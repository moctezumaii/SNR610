...
# Exploring the CLT {#sec-ass2}

The applet on [Ward and Nolte's book](https://collinn.github.io/sbi/ch7.html#central-limit-theorem) [@ward2024] is a great example of the central limit theorem. If you still have questions about this topic I recommend you check the applet on section 6.3 and the one on section 7.2. going to do a little experiment regarding the Central Limit Theorem and confidence intervals. We are going to:
...
Now, I want you to do this 100 times (take a sample, estimate the CI, check if the real mean is included). This is just like in the applet from the book chapter you read earlier.
...
I agree with all of that. However, as you work through the class materials I hope you will understand all the sections of the plot and make nicer plots than this!
...
If you run the test using this alternative way, the t value of the intercept actually corresponds to the t value of a t-test. Essentially, r is running a t-test. Hopefully, this is the way you will be running t-tests in the future. In this case the absolute value of t (t=-1.55) is lower than the critical value. What does that mean?
...
Now, we can also run a t-test using the `t.test` function
