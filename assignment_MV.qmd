...
It has been fun making these assignments, less fun having to grade them! This is the last assignment in the sequence, and it is a *relatively* short one 😄
...
Check the help file for varespec:
...
2- Replicate one of the screeplots but instead of the eigenvalues, use the proportion of variance explained. A barplot or line plot, but you should estimate proportion, and plot it :)
:::

Let's plot the sites in a two-dimensional space (using PC1 and PC2):
...
However, the prebuilt plots are in my mind, a bit ugly. If you will be working with multivariate data, I recommend you spend some time to make sure your plots are nice and easy to interpret!
...
And we can get the eigenvalues like this:
...
Try to interpret the last plot. As if you were describing the importance of the results in a paper.
:::
...
Let's run a quick NMDS with the same dataset. Again, reading the vegan documentation is important in order to fully understand this. Here, I have decided to keep it very brief.
...
If you are wondering how stress is calculated, it is calculated using the following equation:
